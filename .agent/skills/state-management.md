...
Every API service MUST be placed in its own feature folder inside `src/store/Api/` and contain a corresponding types file:

```
src/store/Api/Product/
├── product.api.ts           # API endpoints
└── product.type.ts          # Types (MUST exist)
```
...
```tsx
// src/store/Api/Product/product.type.ts
...
```tsx
// src/store/Api/Product/product.api.ts
...
- [ ] Create `src/store/Api/[Feature]/[feature].api.ts`
- [ ] Create `src/store/Api/[Feature]/[feature].type.ts`
- [ ] Add tag type to `src/store/Api/BaseApi/BaseApi.ts`
- [ ] Export hooks from API file
- [ ] Use LIST pattern for cache tags
- [ ] Import types from type file in components
